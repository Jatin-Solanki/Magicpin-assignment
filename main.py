...
app = FastAPI(title="Vera", version="11.0.1", lifespan=lifespan)
START = time.time()
...
# Semaphore(2): Mistral has 1 RPS handled by _mistral_last_call sleep; allows one Mistral +
# one Groq/OpenRouter call in parallel. Semaphore(1) was over-serializing and wasting budget.
...
DEFAULT_DECISIONS = {
    ...
    "customer_lapsed_hard": "Send the winback today before the customer goes cold permanently",
    ...
    "perf_spike": "Capitalize on the momentum today before the spike fades",
    "perf_dip": "Reverse the drop today before more revenue is lost",
    "dormant_with_vera": "Re-engage today because lapsed customers are being lost every day",
    "milestone_reached": "Celebrate the milestone and launch a thank-you offer today",
    "cde_opportunity": "Register for the CDE session today before spots fill up",
    "price_gap": "Match competitor pricing today before nearby bookings are lost",
}
# ─── LLM call ────────────────────────────────────────────────────────────────
...
def repair_body(body: str, facts: dict) -> str:
    ...
    if locality and locality.lower() not in body.lower():
        ...
        if m:
            ...
            # Fallback: inject locality after the first word group (name greeting)
            m2 = re.match(r"^((?:Dr\.?\s+)?\w[\w\s]{0,30})\s+(.*)", body)
            if m2:
                body = f"{m2.group(1)}, {locality}: {m2.group(2)}"
    ...
    # Ensure at least one core performance metric appears explicitly
    perf_mentioned = False
    for label, value in (("views", facts.get("views_30d")), ("calls", facts.get("calls_30d"))):
    ...
    if ctr:
        try:
            if f"{float(ctr):.1%}" in body or f"{float(ctr):.0%}" in body or str(ctr) in body:
                perf_mentioned = True
        except Exception:
            pass
    if not perf_mentioned:
        for label, value in (("views", facts.get("views_30d")), ("calls", facts.get("calls_30d"))):
            if value and str(value).lower() not in body.lower():
                body = body.rstrip(" .?!") + f" ({value} {label})."
                break
...
# ─── All trigger kinds handled by build_message_plan ─────────────────────────
_KNOWN_KINDS = {
    ...
    "active_planning_intent", "customer_lapsed_hard",
    "customer_lapsed_soft", "trial_followup", "chronic_refill_due",
    "category_seasonal", "price_gap", "seasonal_perf_dip",
}
...
def build_grounded_fallback(category: dict, merchant: dict, trigger: dict,
    ...
    if kind == "research_digest" and (facts["digest_title"] or matching_digest_item(facts)):
        ...
        views = facts.get("views_30d", "0")
        if lapsed:
            body = f"{name}, {locality}: {lapsed} customers inactive 180+ days. Profile has {views} views — re-engagement plan drafted. Activate today? Reply YES / STOP"
        else:
            body = f"{name}, {locality}: {views} views but repeat footfall is slow. Activate a 10% loyalty offer to bring customers back today? Reply YES / STOP"
        ...
        body = f"{name}, {rule} is now mandatory for your {locality} practice. Deadline: {deadline}. 3 nearby clinics have already complied. Want the checklist today? Reply YES / STOP"
        ...
        body = f"{name}, {locality} hit {value} today. Use this momentum for a thank-you flash offer — draft is ready. Launch now? Reply YES / STOP"
        ...
        body = f"{name}, {segment} in {locality} are searching for {opportunity}. You're positioned to capture this — set up a campaign today? Reply YES / STOP"
        ...
        body = f"{name}, urgent update{product_str}{batch_str} in {locality}. Action needed — want the full details today? Reply YES / STOP"
        ...
        body = f"{name}, {name_str}{med_str} refill is{due_str}. Want me to send them a reminder today? Reply YES / STOP"
        ...
        views = facts.get('views_30d', '0')
        body = f"{name}, {locality}: {season} demand shift detected. Top items: {top3}. {views} views last month show high intent — trending push drafted. Activate today? Reply YES / STOP"
        ...
        body = f"{name}, {draft_str}launching {program} in {locality} could open a new revenue stream. Review the plan today? Reply YES / STOP"
        ...
        body = f"{name}, {name_str} tried {service} recently. {next_step} offer drafted to convert them — send now before interest drops? Reply YES / STOP"

    elif "lapsed" in kind:
        customer_name = facts.get("customer_name", "")
        days_gone = payload.get("days_since_last_visit", "")
        name_str = customer_name or "a lapsed customer"
        body = f"{name}, {name_str} hasn't visited in {days_gone} days. Winback offer drafted — send today before they go cold? Reply YES / STOP"

    elif "price_gap" in kind or "price" in kind:
        item = (payload.get("item_name") or payload.get("item") or "service").replace("_", " ")
        your_p = payload.get("your_price") or payload.get("price") or ""
        comp_p = payload.get("competitor_price") or payload.get("category_median") or ""
        body = f"{name}, {locality}: {item} price gap — you at Rs.{your_p}, competitor at Rs.{comp_p}. Match today before bookings shift? Reply YES / STOP"

    elif offer:
        body = f"{name}, {offer} is live in {locality}. Want to see how it's performing vs peers? Reply YES / STOP"
    else:
...
def validate_body(body: str, merchant: dict, facts: dict | None = None,
    ...
    if facts:
        ...
        if cat == "dentists" and not low.startswith("dr. ") and not low.startswith("dr "):
...
MANDATORY 3-PART STRUCTURE - every message MUST have all 3 parts in order:
1. [Name/Prefix + sharpest fact RIGHT NOW]
2. [Current merchant state with numbers]
3. [One specific decision for TODAY + binary CTA]

GOLD STANDARD EXAMPLES BY CATEGORY:
...
WHY: Dr. prefix, source/stat/trial_n, merchant cohort number, current state, one decision, binary CTA.

RESTAURANT:
"Rajesh, Sector 18: covers dropped 32% this week to 180 from 265 baseline. Your 'Family Thali' offer is live — boost it before tonight's dinner rush? Reply YES / STOP"
WHY: Owner name, locality, specific delta+baseline numbers, named offer, time-sensitive, binary CTA.

SALON:
"Priya, GK-2: 87 bookings this month but 23 bridal inquiries unanswered. Your 'Bridal Glow' package is ready — activate campaign before wedding season peaks? Reply YES / STOP"
WHY: Name, locality, booking numbers, specific package name, seasonal urgency, binary CTA.

GYM:
"Vikas, Indiranagar: 34 trial leads this week, only 8 converted. Member retention at 61% — your '3-Month Transform' offer can close the gap today? Reply YES / STOP"
WHY: Name, locality, trial+conversion numbers, retention %, named offer, binary CTA.

PHARMACY:
"Sharma Ji, Lajpat Nagar: 156 chronic refills due this week, 38 patients overdue 30+ days. Stock alert on Metformin batches — reorder before compliance gap widens? Reply YES / STOP"
WHY: Name, locality, refill+overdue numbers, specific molecule, compliance angle, binary CTA.

BAD EXAMPLE (NEVER WRITE LIKE THIS):
"Hi! I hope you're doing well. I wanted to reach out about your business performance. Would you like to know more? Reply YES / STOP"
WHY IT FAILS: "Hi" opening, no name, no locality, zero numbers, no specific fact, no urgency, generic question.
...
HARD RULES:
1. body <= 320 characters - count every character including spaces
2. NO URLs, NO jargon, NO fluff, NO emojis
3. Use ONLY anchor facts in current prompt - never invent numbers or names
4. Name/Title first (Dr. or owner name) - NEVER start with Hi/Hello/Vera/I hope/Hey
5. ONE CTA ONLY at the very end - exact ending must be "Reply YES / STOP"
6. Use >=3 anchor facts, including >=3 numbers/dates/sources from the facts provided
7. CURRENT STATE phrase required: show merchant situation RIGHT NOW with data
...
10. COMPULSION LEVER: follow the assigned lever exactly
    effort_externalization -> say the work is drafted/pulled/ready for one-tap action
    loss_aversion -> quantify what is being lost right now with numbers
    curiosity -> ask for the full picture/breakdown with specific reference
    social_proof -> cite peer benchmark numbers from anchor facts
...
def compose(category: dict, merchant: dict, trigger: dict,
    ...
    coro = safe_compose(category, merchant, trigger, customer, turn=1)
    try:
...
HOSTILE_PHRASES = [
    ...
    "nahi chahiye", "band karo", "mat bhejna", "mat bhejo",
    "bilkul nahi", "mat karo", "block kar", "report kar",
    "remove me", "block me", "not now stop",
]
REJECT_PHRASES = [
    "not interested", "no thanks", "nahi", "nope",
    "no need", "don't need this", "please stop",
    "nahi chahiye", "koi zaroorat nahi", "abhi nahi",
]
COMMIT_PHRASES = [
    ...
    "theek hai", "kar do", "haanji", "bhej do", "kar dijiye",
    "ok do it", "sure do it", "absolutely", "definitely",
]
...
def detect_intent(msg: str) -> str:
    ...
    # "stop" at start of message — likely opt-out even with extra words
    if low.startswith("stop ") and len(low) < 30:
        return "hostile"
    ...
    # Bare single-word affirmations (exact match only)
    if low.rstrip(".!?") in ("yes", "ok", "okay", "sure", "haan", "ji", "haanji"):
        return "committed"
...
async def metadata():
    return {
        "team_name": "Jatin Solanki",
        "team_members": ["Jatin Solanki"],
...
async def reply(body: ReplyBody):
    ...
    if intent == "committed":
        ...
        action_user = (
            ...
            f"Locality: {locality}\n"
            f"Customer (if applicable): {customer_name}\n"
            f"Active offers: {active_offers}\n"
            f"Task: Deliver the promised next step in ≤160 chars. "
            f"Reference specific details from the trigger (e.g., patient name, offer name, "
            f"campaign topic). Use ACTION words: 'Sending now', 'Here it is', 'Done', 'Confirmed'. "
            f"NO qualifying questions. NO URLs. "
        ...
        if not reply_body or reply_body == last_bot_msg:
            # Trigger-specific fallback for committed action
            COMMITTED_ACTION_FALLBACKS = {
                "recall_due": f"Done{', ' + merchant_name if merchant_name else ''}! Sending the recall reminder{' for ' + customer_name if customer_name else ''} now.",
                "research_digest": f"On it{', ' + merchant_name if merchant_name else ''}! Drafting the protocol update based on the new evidence now.",
                "renewal_due": f"Confirmed{', ' + merchant_name if merchant_name else ''}! Starting the renewal process for your listing in {locality}." if locality else f"Confirmed{', ' + merchant_name if merchant_name else ''}! Starting the renewal process now.",
                "festival_upcoming": f"Done! Activating the festival campaign{', ' + merchant_name if merchant_name else ''} — draft is being sent now.",
                "perf_dip": f"On it{', ' + merchant_name if merchant_name else ''}! Pulling the boost plan for your {locality} listing now." if locality else f"On it{', ' + merchant_name if merchant_name else ''}! Pulling the performance boost plan now.",
                "supply_alert": f"Confirmed{', ' + merchant_name if merchant_name else ''}! Sending the quarantine checklist for affected batches now.",
            }
            reply_body = COMMITTED_ACTION_FALLBACKS.get(
                trigger_kind,
                f"On it{', ' + merchant_name if merchant_name else ''}! Sending the details now."
            )
